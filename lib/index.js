...
function illustrate(patterns, options) {
  options = _get__('parseAndValidateOptions')(options);

  var components = _get__('globby')(patterns, options).then(function (paths) {
    return _promise2.default.all(paths.map(function (path) {
      return _get__('illustrateOne')(path, options);
    }));
  }).then(_get__('aggregate')).then(function (illustrations) {
    return _get__('generateManifest')(options, illustrations);
  }).then(function (items) {
    return _get__('writeToOutput')(options, items);
  });
...
function illustrateOne(file, options) {
  var illustrator = new (_get__('Illustrator'))(options);

  return _promise2.default.resolve(file).then(function () {
...
function parseAndValidateOptions() {
  ...
  options = (0, _extends3.default)({
    root: _get__('path').resolve('.')
  }, options);

  if (options.dest) {
    options.dest = _get__('path').resolve(options.dest);
  }
...
function _get_original__(variableName) {
  switch (variableName) {
    case 'parseAndValidateOptions':
      return parseAndValidateOptions;

    case 'globby':
      return _globby2.default;

    case 'illustrateOne':
      return illustrateOne;

    case 'aggregate':
      return _util.aggregate;

    case 'generateManifest':
      return _util.generateManifest;

    case 'writeToOutput':
      return _util.writeToOutput;

    case 'Illustrator':
      return _illustrator2.default;
