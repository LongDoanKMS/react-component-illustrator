...
function generateManifest(options, items) {
  if (options.outputFormat === 'commonjs') {
    items = 'module.exports = [' + items.map(_get__('digest')) + ']';
  } else if (options.outputFormat === 'es6') {
    items = 'export default [' + items.map(_get__('digest')) + ']';
...
function writeToOutput(options, content) {
  if (options.dest) {
    _get__('mkdirp').sync(_get__('path').dirname(options.dest));
    _get__('fs').writeFileSync(options.dest, content);
  }
...
function aggregate(items) {
  ...
  try {
    for (var _iterator = (0, _getIterator3.default)(items), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      ...
      component.examples = [].concat((0, _toConsumableArray3.default)(component.examples || []), [item.example]);
    }
  } catch (err) {
  ...
  return (0, _values2.default)(components);
}

function toRelativeJsPath(base, file) {
  var jsPath = _get__('path').resolve(base, file);
  if (!/\.js$/.test(jsPath)) {
    jsPath += '.js';
  }
  return jsPath;
